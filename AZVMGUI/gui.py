...
class AzureVMCreator:
    ...
    def setup_variables(self):
        ...
        self.resource_group = tk.StringVar(value="myResourceGroup")
        ...
        self.create_resource_group = tk.BooleanVar(value=True)
    ...
    def create_basic_settings_frame(self, parent):
        ...
        ttk.Entry(frame, textvariable=self.resource_group, width=40).grid(row=1, column=1, sticky=tk.W, padx=(5, 0))
        ttk.Checkbutton(frame, text="Erstellen falls nicht vorhanden", 
                       variable=self.create_resource_group).grid(row=1, column=2, padx=(10, 0))
        
        # Location
        ttk.Label(frame, text="Location:").grid(row=2, column=0, sticky=tk.W, pady=2)
        ...
        # VM Size
        ttk.Label(frame, text="VM Größe:").grid(row=3, column=0, sticky=tk.W, pady=2)
        ...
        # Image
        ttk.Label(frame, text="Betriebssystem:").grid(row=4, column=0, sticky=tk.W, pady=2)
        image_combo = ttk.Combobox(frame, textvariable=self.image, width=37)
        image_combo['values'] = ('Ubuntu2204', '2025-datacenter-azure-edition', 'Win2022Datacenter', 
                                'Win2019Datacenter', 'CentOS85Gen2', 'Debian11')
    ...
    def check_azure_cli(self):
        ...
        def run_check():
            ...
            try:
                # Windows: Versuche verschiedene Pfade
                az_command = "az"
                if os.name == 'nt':  # Windows
                    # Prüfe ob az.cmd im PATH ist
                    try:
                        subprocess.run([az_command, "--version"], 
                                     capture_output=True, text=True, timeout=2)
                    except FileNotFoundError:
                        # Versuche az.cmd explizit
                        az_command = "az.cmd"
                
                result = subprocess.run([az_command, "--version"], 
    ...
    def generate_command(self):
        ...
        command_parts = [az_command, "vm", "create"]
    ...
    def create_vm(self):
        ...
        if not messagebox.askyesno("Bestätigung", 
                                  f"Möchten Sie die VM '{self.vm_name.get()}' erstellen?"):
            return
        
        def run_creation():
            self.log_output(f"=== VM Erstellung gestartet: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ===")
            
            # Azure CLI Befehl bestimmen (Windows-Kompatibilität)
            az_command = "az"
            shell_flag = False
            if os.name == 'nt':  # Windows
                az_command = "az.cmd"
            ...
            try:
                # Resource Group erstellen falls erforderlich
                if self.create_resource_group.get():
                    self.log_output("Resource Group wird erstellt...")
                    rg_command = [
                        az_command, "group", "create",
                        "--name", self.resource_group.get(),
                        "--location", self.location.get()
                    ]
                    
                    result = subprocess.run(rg_command, capture_output=True, text=True, 
                                          timeout=60, shell=shell_flag)
                    if result.returncode == 0:
                        self.log_output("✓ Resource Group erstellt oder bereits vorhanden")
                    else:
                        self.log_output(f"✗ Fehler beim Erstellen der Resource Group: {result.stderr}")
                        return
