...
class BewerbungsManager:
    ...
    def create_widgets(self):
        ...
        labels = ["Firma:", "Position:", "Ansprechpartner:", "Datum (TT.MM.JJJJ):", "Status:", "Notizen:"]
        ...
        self.entry_datum = tk.Entry(frame_form, width=20)
    ...
    def lade_daten(self):
        """Lädt alle Bewerbungen aus der CSV"""
    ...
    def speichern(self):
        """Speichert oder fügt neue Bewerbung hinzu"""
        daten = [
            self.entry_firma.get().strip(),
            self.entry_position.get().strip(),
            self.entry_ansprech.get().strip(),
            self.entry_datum.get().strip(),
            self.status_var.get(),
            self.entry_notizen.get().strip()
        ]

        if not all(daten[:2]):
            messagebox.showwarning("Eingabe fehlt", "Bitte mindestens Firma und Position angeben!")
            return
    ...
    def auswahl_anzeigen(self, event):
        """Zeigt die markierte Bewerbung in den Eingabefeldern"""
        ...
        self.entry_datum.insert(0, werte[3])
        self.status_var.set(werte[4])
        self.entry_notizen.delete(0, tk.END)
        self.entry_notizen.insert(0, werte[5])

    def bearbeiten(self):
        """Aktualisiert die ausgewählte Bewerbung"""
        auswahl = self.tree.focus()
        if not auswahl:
            messagebox.showinfo("Keine Auswahl", "Bitte eine Bewerbung auswählen.")
            return

        werte_alt = self.tree.item(auswahl, "values")
        neue_werte = [
            self.entry_firma.get().strip(),
            self.entry_position.get().strip(),
            self.entry_ansprech.get().strip(),
        ...
        with open(DATEI, newline="", encoding="utf-8") as f:
            ...
            header = next(reader)
            for row in reader:
                if tuple(row) == werte_alt:
                    daten.append(neue_werte)
                else:
                    daten.append(row)
    ...
    def filtern(self):
        """Filtert Bewerbungen nach Suchbegriff"""
