...
class SoundButton:
    def __init__(self, master, index, config):
        ...
        self.reverse_enabled = config.get("reverse", False)
        ...
        self.reverse_var = tk.BooleanVar(value=self.reverse_enabled)
        self.reverse_checkbox = tk.Checkbutton(master, text="Rückwärts", variable=self.reverse_var)
        self.reverse_checkbox.grid(row=index, column=3, padx=5, pady=5)
    ...
    def play_sound(self):
        try:
            if self.reverse_var.get():
                seg = AudioSegment.from_file(self.sound_path)
                reversed_seg = seg.reverse() - (60 - int(self.scale.get() * 60))
            ...
            print(f"Fehler beim Abspielen von Sound: {e}")
        self.is_playing = False
...
class SoundApp:
    def __init__(self, root):
        ...
        self.quit_button.grid(row=10, column=0, columnspan=4, pady=10)
